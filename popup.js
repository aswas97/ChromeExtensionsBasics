    ...
    for (let i = 0; i < windows.length; i++) {
        ...
        saveButton.addEventListener('click', function () {
            ...
            chrome.storage.sync.set({ [instanceName]: urls }, function () {
                // After saving the instance, refresh the saved instances list.
                displaySavedInstances();
...
function displaySavedInstances() {
    ...
    chrome.storage.sync.get(null, function (items) {
        ...
        for (let key in items) {
            ...
            openButton.addEventListener('click', function () {
                chrome.windows.create({ url: items[key].map(tab => tab.url) });
            });
            ...
            deleteButton.addEventListener('click', function () {
                chrome.storage.sync.remove(key, function () {
                    displaySavedInstances(); // Reload the saved instances list after deletion
                    // After saving the instance, refresh the saved instances list.
